...
To use the customizations here yourself, /you must change the name of
 ...
 is 'lalor', so this file is named =lalor.org=, and when Emacs
 starts up with the starter kit it creates the file =lalor.el=.
 
You will also need to change the values for the location of bibtex
 databases, pandoc and orgmode-mobile specified below, and some other
 settings. *Search for 'lalor' in this file* to find the paths that
...
I'm turning this bit of code off as I don't use a Mac (so no Skim).
#+srcname: sync
#+begin_src emacs-lisp :tangle no
    ;; Synctex with Skim
...
Create =html= files from the =org= sources, to help with documentation. To set up org-mode for publishing projects to HTML you will need to change these settings, as they apply only to the Starter Kit. 
...
*** org-mode paper notes settings
This will set up a workflow for reading papers and taking notes in org-mode. 
Based on: https://www.reddit.com/r/emacs/comments/4gudyw/help_me_with_my_orgmode_workflow_for_notetaking/ (Alternate workflow). 

#+source: org-mode-paper-notes
#+begin_src emacs-lisp 
...
** Local Mail Settings
I use mu4e for reading mail. 

*** Reading Mail 
#+source:mu4e-configuration
#+begin_src emacs-lisp
...
;;store link to messages for to-dos
;;(require 'org-mu4e)
...
#+begin_src emacs-lisp 
  ...
  (setq
     user-mail-address "john.lalor@nd.edu"
     user-full-name  "John Lalor"
  ...
  ;;(setq mu4e-compose-keep-self-cc f)
  ...
  (setq message-send-mail-function 'smtpmail-send-it
     ...
     smtpmail-auth-credentials
       '(("smtp.gmail.com" 587 "jlalor1@nd.edu" nil))
     smtpmail-stream-type 'starttls
     smtpmail-default-smtp-server "smtp.gmail.com"
     smtpmail-smtp-server "smtp.gmail.com"
     smtpmail-smtp-service 587
     smtpmail-smtp-user "jlalor1@nd.edu")

  ;;(setq auth-source-xoauth2-creds '~/.passwd/smtp.hash)
  ;;(add-to-list 'smtpmail-auth-supported 'xoauth2)
  ;;(auth-source-xoauth2-enable)

  ;;(setq auth-source-xoauth2-use-curl t)
  ...
  ;; update mode line unread emails
...
#+end_src 
** Local Elfeed Settings
Elfeed is an RSS reader. 

#+source: elfeed-configuration
#+begin_src emacs-lisp
(setq elfeed-db-directory "~/org/elfeeddb") 

(defun lalor/elfeed-show-all ()
  (interactive)
  (bookmark-maybe-load-default-file)
  (bookmark-jump "elfeed-all"))

(defun lalor/elfeed-load-db-and-open ()
  (interactive)
  (elfeed)
  (elfeed-db-load)
...
(defun lalor/elfeed-save-db-and-bury ()
  (interactive)
  (elfeed-db-save)
  (quit-window))
...
("q" . lalor/elfeed-save-db-and-bury)
("Q" . lalor/elfeed-save-db-and-bury)
